...
# 写入 CSV 文件
def write_results_to_csv(filename: str,  results: List[Tuple], model_name=None):
    """Write test results to a CSV file with header check and append mode."""
    header = [
        # 'Model Name', 'TP',"PP", 'Quant Type', "Test Data", 'Max Model Len','GMZ',
                    ...
                    async for chunk_bytes in response.content:
                        ...
                        if chunk != "[DONE]":
                            ...
                            if choices := data.get("choices"):
                                ...
                                prompt_len = usage.get("prompt_tokens")
...
async def async_request_ksana_generate(
    request_func_input: RequestFuncInput,
    pbar: Optional[tqdm] = None,
) -> RequestFuncOutput:
    api_url = request_func_input.api_url
    ...
    async with _create_bench_client_session() as session:
        payload = {
            ("prompt" if isinstance(prompt, str) else "input_tokens"): prompt,
            "sampling_config": {
                "temperature": 0.0,
                "max_new_tokens": request_func_input.output_len,
                "ignore_eos": not args.disable_ignore_eos,
...
def get_dataset(args, tokenizer):
    ...
    if args.dataset_name == "sharegpt":
        ...
        assert not tokenize_prompt
        input_requests = sample_generated_custom_dataset(
            num_groups=args.gsp_num_groups,
            prompts_per_group=args.gsp_prompts_per_group,
            system_prompt_len=args.gsp_system_prompt_len,
            question_len=args.gsp_question_len,
...
def gen_prompt(tokenizer, token_num):
    ...
    # re_encoded_sequence = tokenizer.encode(prompt, add_special_tokens=False)[:token_num]
    re_encoded_sequence = tokenizer.encode(prompt)[:token_num]
    prompt = tokenizer.decode(re_encoded_sequence)
    # print(token_num, len(selected_tokens), len(tokenizer.encode(tokenizer.decode(selected_tokens))), len(tokenizer.encode(prompt)))
    return prompt


def get_gen_prefix_cache_path(args, tokenizer):
    """Create cache directory under ~/.cache/perf/benchmark"""
    cache_dir = Path.home() / ".cache" / "perf" / "benchmark"
...
def sample_generated_custom_dataset(
    num_groups: int,
    ...
    question_len = question_len -1 # Adjust for BOS token
    questions = []
    with open(args.dataset_path, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            context_len = question_len-len(promt_ids)
            ...
            if new_len == question_len:
                questions.append(new_input)

    total_num_promt = num_groups * prompts_per_group           
    assert total_num_promt <= len(questions)
...
def calculate_metrics(
    ...
    tps_requests_i: List[float] = []
    tps_requests_o: List[float] = []
    tps_requests_i_o: List[float] = []
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            tps_requests_i.append(outputs[i].prompt_len/outputs[i].ttft)
            tps_requests_o.append(output_len/outputs[i].latency)
            tps_requests_i_o.append((output_len+outputs[i].prompt_len)/outputs[i].latency)
    ...
    metrics = BenchmarkMetrics(
        ...
        input_throughput=sum(tps_requests_i)/(completed/max_concurrency),
        output_throughput=sum(tps_requests_o)/(completed/max_concurrency),
        total_throughput=sum(tps_requests_i_o)/(completed/max_concurrency),
...
async def benchmark(
    ...
    # # 将结果保存到列表中
    all_results = [(
            prompt_len, output_len, max_concurrency,
            ...
            round(metrics.mean_e2e_latency_ms, 2),
    ...
    if args.output_file:
        ...
        if args.dataset_name.startswith("random"):
            output_file_name = f"data/{args.backend}_{now}_{args.num_prompts}_{args.random_input_len}_{args.random_output_len}.jsonl"
        else:
            output_file_name = f"data/{args.backend}_{now}_{args.num_prompts}_sharegpt.jsonl"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="sharegpt",
        choices=["sharegpt", "random", "random-ids", "generated-shared-prefix", "mmmu", "custom"],
