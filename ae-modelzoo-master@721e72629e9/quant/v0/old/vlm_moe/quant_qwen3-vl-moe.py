...
def main(input_path, output_path, quant_type, version):
    ...
    custom_ignores = ['lm_head', 'visual', 'gate.weight', 'norm', 'bias', 'embed_tokens']
    ...
    for safetensor_file in tqdm(safetensor_files, desc=f"Converting to {quant_type}"):
        ...
        for weight_name, weight in current_state_dict.items():
            ...
            if "expert" in weight_name:
                ...
                temp_i8_s = []
                temp_i8_z = []
                assert weight.shape[0] == 128, f"{weight.shape}"
                for i in range(weight.shape[0]):
                    weight_e = weight[i].transpose(0, 1)  # 转置1次
                    if quant_type == "w4a8":
                        quant_weights, scale, i8scales, i8zeros = weight_quant_moe_int4_pack8(weight_e)
                    elif quant_type == "w8a8":
                        quant_weights, scale = weight_quant_int8(weight_e)
                ...
                if quant_type == "w4a8":
                    new_weights = torch.stack(temp_w).transpose(1, 2).contiguous() # 转置回去
                    scales = torch.stack(temp_s).transpose(1, 2).contiguous()  # scale 也要转置
                else:
                    new_weights = torch.stack(temp_w).transpose(1, 2).contiguous() # 转置回去
                    scales = torch.stack(temp_s).contiguous()
                # print(weight_name, new_weights.shape, scales.shape)
                # input()
                if temp_i8_s:
                    new_state_dict[i8scales_name] = torch.stack(temp_i8_s)
                if temp_i8_z:
                    new_state_dict[i8zeros_name] = torch.stack(temp_i8_z)
