...
def parse_layer_index(weight_name):
...
def is_quant_layer(weight_name, quant_layers):
    layer_index = parse_layer_index(weight_name)
    if layer_index is not None and layer_index in quant_layers:
        return True
    return False
...
def weight_quant_moe(
    ...
    i4_weights = i4_weights.squeeze(0)
    return i4_weights, scale.to(torch.float32), i8scales, i8zeros
...
def main(
    ...
    for safetensor_file in tqdm(safetensor_files):
        ...
        for weight_name, weight in current_state_dict.items():
            if not is_quant_layer(weight_name, quant_layers):
                print(weight_name)
                new_state_dict[weight_name] = weight
                new_weight_map[weight_name] = file_name

            elif "experts" in weight_name:
                i4_weights, scale, i8scales, i8zeros = weight_quant_moe(
                    weight, 128, group_size, format, symmetric, version
                )
                if version == 2:
            ...
            elif "proj" in weight_name and "bias" not in weight_name:
                if gemm_quant_method == "w8a8":
                    int8_v, scale = weight_quant(weight)
                elif gemm_quant_method == "w4a8":
                    # if weight_name == "model.layers.20.self_attn.q_proj.weight":
                    #     import ipdb; ipdb.set_trace()
                    int8_v, scale, _, _ = weight_quant_moe(weight, format="NN")
                else:
                    raise ValueError(f"Unknown gemm quant method: {gemm_quant_method}")
    ...
    for other_file in other_files:
        if os.path.isfile(other_file):
            ...
            shutil.copy(os.path.join(other_file), os.path.join(int8_path, name))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--quant-layers", type=int, nargs="+", default=[])
    parser.add_argument("--gemm-quant-method", type=str, default="w8a8")
    args = parser.parse_args()
    main(
        args.input_fp8_hf_path,
        args.output_int8_hf_path,
        args.group_size,
        args.format,
        args.symmetric,
        args.version,
