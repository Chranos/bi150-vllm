...
class DynamicQuantizeLinear(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = w8a8(inputs, self.weight, i_scales,
                   self.scale, self.bias, output)
...
def _is_linear_wan(mod, *args):
    return isinstance(mod, torch.nn.Linear) and ("condition_embedder" not in args[0])
...
def apply_quant_linear_i8w8o16(model, cls=DynamicQuantizeLinear, filter_fn=None):
    ...
    if type(model).__name__ == "xFuserWanTransformer3DModelWrapper":
        filter_fn = _is_linear_wan
