...
def main():
    ...
    text_encoder_2 = T5EncoderModel.from_pretrained(
        engine_config.model_config.model, subfolder="text_encoder_2", torch_dtype=torch.bfloat16)
    ...
    output = pipe(
        ...
        generator=torch.Generator(
            device="cuda").manual_seed(input_config.seed),
    )
    end_time = time.time()
    ...
    if input_config.output_type == "pil":
        ...
        dp_batch_size = (input_config.batch_size +
                         num_dp_groups - 1) // num_dp_groups
