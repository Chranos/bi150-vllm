...
def get_kv_cache_config_from_groups(
    ...
    if len(kv_cache_groups) == 1 and isinstance(
        ...
        kv_cache_scale_tensors = []
        if envs.VLLM_KV_DISABLE_CROSS_GROUP_SHARE:
            # Debug mode for isolation: no cross-group sharing, each layer gets
            # its own KV tensor to avoid aliasing between attention types.
            for group in kv_cache_groups:
                for layer_name in group.layer_names:
                    ...
                    kv_cache_scale_tensors.append(
                        KVCacheTensor(
                            size=scale_page_size * num_blocks,
                            shared_by=[layer_name],
                        )
                    )
            logger.warning(
